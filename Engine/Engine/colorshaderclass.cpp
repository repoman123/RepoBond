...
bool ColorShaderClass::Initialize(ID3D10Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd,L"../Engine/Shaders/color.fx");
	if(!result)
	{
		return false;
	}
...
void ColorShaderClass::Render(ID3D10Device* device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,D3DXVECTOR4 color)
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix);
	mColor->SetFloatVector((float*)color);
...
bool ColorShaderClass::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
	...
	mColor = m_effect->GetVariableByName("color")->AsVector();
	return true;
}


void ColorShaderClass::DisposeShader()
{
